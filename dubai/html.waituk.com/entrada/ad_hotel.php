...
<span style="font-size:30px;cursor:pointer; color:white;" class="nav2"  >&#9776; Dashboard</span>
	...
	?>
  <a href="ad_hotel_insert.php"><button type="button" class="btn btn-info" style="text-align: right;"><i class="fa fa-plus" aria-hidden="true"></i></button></a>
</div>
	...
	<br/>
  <center>
    <h4 style="color:#68FF33";>Hotel table </h4>
</center>
    <?php
 ...
 $result = mysqli_query($con, $query);  
  /*
$selhotel="select * from Hotel limit $start_from,$num_per_page";
$reshotel=mysqli_query($con,$selhotel);
$n=mysqli_num_rows($reshotel);
*/
for($page = 1; $page<= $number_of_page; $page++) {  
  echo '<a href = "ad_hotel.php?page=' . $page . '">' . $page . ' </a>';  
}  
...
{ $i++;
//    $row=mysqli_fetch_row($reshotel);
    echo"<tr style='padding-top: 60px;'> 
              <td style='padding-top: 20px;'>$i
              <td style='padding-top: 20px;'>$row[1]
              <td style='padding-top: 20px;'>$row[2]
              <td style='padding-top: 20px;'><img src='hotel_img/$row[3]' width='100px' alt='image'>
              <td><a href='ad_hotel_delete.php?uid=$row[0]'><button type='button' class='btn btn-danger'><i class='fa fa-trash'></i></button></a>
              <td><a href='ad_hotel_update.php?uid=$row[0]'><button type='button' class='btn btn-warning'><i class='fa fa-pencil'></i></button></a>";
}   
